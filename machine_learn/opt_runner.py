...
from core.opt_train import train_net
...
def objective(trial):
    alpha = trial.suggest_uniform('alpha', 1e-5, 1e-4)
    beta = trial.suggest_uniform('beta', 1e-5, 1e-4)
    #return train_net(cfg, alpha=alpha, beta=beta)
    encoder_weight_decay = trial.suggest_uniform('encoder_weight_decay ', 1e-2, 1e-1)
    decoder_weight_decay = trial.suggest_uniform('decoder_weight_decay ', 1e-2, 1e-1)
    return train_net(cfg, alpha=alpha, beta=beta, encoder_weight_decay=encoder_weight_decay, decoder_weight_decay=decoder_weight_decay)
...
def main():
    ...
    # https://api.slack.com/apps/A02SSPEJV9S/incoming-webhooks?success=1
    slack = slackweb.Slack(url="https://hooks.slack.com/services/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")
    elapsed_time = time.time() - start
    slack.notify(text="学習完了 学習時間"+str(elapsed_time)+"秒")
